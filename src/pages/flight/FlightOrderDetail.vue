      ...
      >
        <div class="card">
          ...
          <table class="table table-hover table-sm mb-0">
            ...
            <tbody>
              <tr
                v-for="(info, index) in detail.passengers"
                :key="index"
                class="bg-white"
              >
                <td class="text-right">
                ...
                <td>
                  <template
                    v-if="
                      detail.orderStatus === 16 || detail.orderStatus === 32
                    "
                  >
                    {{ info.ticketNo }}
                    <template v-if="info.ticketCount > 1">
                      - {{ info.ticketNoEnd }}</template
                    >
                  </template>
                </td>
                <td>
          ...
          <div class="card-body py-0">
            ...
            >
              ...
              <div class="p-2 text-nowrap">
                票面价: {{ this.adultPrice.price }}
              </div>
              <div class="p-2 text-nowrap">
                销售价: {{ this.adultPrice.parvalue }}
              </div>
              <div class="p-2 text-nowrap">税: {{ this.adultPrice.tax }}</div>
              <div class="p-2 text-nowrap">
                保险费: {{ this.adultPrice.insurance }}
              </div>
              <div class="p-2 text-nowrap">
                服务费: {{ this.adultPrice.serviceCharge }}
              </div>
              <div class="p-2 text-nowrap">
                客户让利: {{ this.adultPrice.discount }}
              </div>
              <div class="p-2 text-nowrap">
                人数: {{ this.adultPrice.ticketCount }}
              </div>
              <div class="p-2 text-nowrap">
                应收: {{ this.adultPrice.amount }}
              </div>
            </div>
            <div
              class="d-flex flex-row justify-content-between"
            ...
            >
              ...
              <div class="p-2 text-nowrap">
                销售价: {{ detail.chdParvalue }}
              </div>
              <div class="p-2 text-nowrap">税: {{ detail.chdTax }}</div>
              <div class="p-2 text-nowrap">
                保险费: {{ detail.chdInsurance }}
              </div>
              <div class="p-2 text-nowrap">
                服务费: {{ detail.chdServiceCharge }}
              </div>
              <div class="p-2 text-nowrap">
                客户让利: {{ detail.chdDiscount }}
              </div>
            ...
            >
              ...
              <div class="p-2 text-nowrap">
                销售价: {{ detail.infParvalue }}
              </div>
              <div class="p-2 text-nowrap">税: {{ detail.infTax }}</div>
              <div class="p-2 text-nowrap">
                保险费: {{ detail.infInsurance }}
              </div>
              <div class="p-2 text-nowrap">
                服务费: {{ detail.infServiceCharge }}
              </div>
              <div class="p-2 text-nowrap">
                客户让利: {{ detail.infDiscount }}
              </div>
              <div class="p-2 text-nowrap">人数: {{ detail.childCount }}</div>
          ...
          <div class="card-body">
            ...
            <ul class="list-inline row">
              <li class="list-inline-item me-auto">
                价格校验: {{ detail.priceCheckedStatus }},
              ...
              <div class="tab-pane fade show" id="newComment">
                <div class="card">
                  <div class="card-body">
                    ...
                    <button class="btn btn-primary" @click.stop="saveComment()">
                      保存
                    </button>
                  </div>
                  <table></table>
