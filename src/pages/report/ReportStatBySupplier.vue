        ...
        <table class="table table-sm table-striped small">
          ...
          <tbody>
            <tr v-for="(info, index) in dataList">
              ...
              <td class="">
                <template v-if="info.supplier !== null">{{info.supplier.name}}</template>
              </td>
...
<script>
  import { searchSupplierStats } from '../../api/report.js'
  ...
  export default {
    ...
    methods: {
      ...
      fillPie: function () {
        ...
        for (const info of this.dataList) {
          this.chartData.rows.push({'供应商': info.supplier.name, '机票数量': info.count})
        }
      },
      reset: function () {
        this.count = 0
