          ...
          <tbody>
            <template v-for="info in dataList">
              <tr :class="{'table-dark': info.status === 4}" style="color: black">
                  ...
                  <td>
                    <router-link :to="`/train/refund/order/` + info.id">详情</router-link>
                  </td>
              </tr>
              <tr v-if="info.remark && info.status !== 4">
...
<script>
  import $ from 'jquery'
  import { searchTrainRefundOrders, showTrainOrderStatus } from '../../api/train.js'
  ...
  export default {
    ...
    methods: {
      ...
      search: function () {
        ...
        searchTrainRefundOrders(params, v => {
